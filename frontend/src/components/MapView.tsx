...
const MapView: React.FC<MapViewProps> = ({
  ...
  return (
    <div className="w-full h-96 rounded shadow overflow-hidden">
      <Map
        initialViewState={{ longitude: userLoc?.lng ?? longitude, latitude: userLoc?.lat ?? latitude, zoom: 12 }}
      ...
      >
        {markers.map((m, i) => (
          <Marker key={i} longitude={m.lng} latitude={m.lat} color="#22c55e" onClick={() => setPopup(m)}>
            {m.label && <div className="bg-white text-xs rounded px-1 py-0.5 shadow cursor-pointer">{m.label}</div>}
          </Marker>
        ))}
        {userLoc && (
